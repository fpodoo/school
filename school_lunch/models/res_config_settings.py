...
class resCompany(models.Model):
    ...
    def _cron_school_lunch_reminder(self):
        ...
        if not menu.search_count([('date', ">=", date.today() + relativedelta(months=1, day=1)), ('date', "<", date.today() + relativedelta(months=2, day=1))]):
            return False
        partners = self.env['res.partner'].search([('kid_ids', '!=', False), ('email','ilike','%')]).filtered(lambda p: any(p.mapped('kid_ids.active')))
...
class ResConfigSettings(models.TransientModel):
    ...
    def send_lunch_email(self):
        self.env.company._cron_school_lunch_reminder()
        return {
