...
class SchoolLunch(http.Controller):
    ...
    def menu_agenda(self, date=None, **kw):
        ...
        menus = request.env["school_lunch.menu"].search(
            [("date", ">=", dt_from.strftime("%Y-%m-%d")), ("date", "<=", dt_to.strftime("%Y-%m-%d"))],
            order="date asc, meal_type desc",
        )
        allergy_ids = {al.id for m in menus for al in m.allergy_ids}
        allergies = request.env["school_lunch.allergy"].browse(list(allergy_ids))

        calendar = defaultdict(lambda: defaultdict(list))
        weekdays = list({menu.date.weekday() for menu in menus})
        latest = None
        weekdays.sort()
        for menu in menus:
        ...
        return http.request.render(
            "school_lunch.website_menu_calendar",
            {
                "allergies": allergies,
                "calendar": calendar,
                "dmonth": relativedelta(months=1),
    ...
    @http.route(
        ["/school/kid/add/<string:uuids>", "/school/kid/add/<string:uuids>/<int:partner_id>"],
        auth="public",
        type="http",
        website=True,
        sitemap=False,
        ...
        for (_meal_type, product, price), orders in meals.items():
            ...
            for order in orders:
                request.env["school_lunch.order"].sudo().create(
                    {"sale_line_id": line_id.id, "menu_id": order[0], "kid_id": order[1]}
                )
    ...
    def school_order_prepare(self, date=None, **kwargs):
        ...
        for menu in menus:
            ...
            if (not len(result["menus"])) or (result["menus"][-1]["date"] != FMT):
                result["menus"].append({"date": FMT, "day_of_week": menu.date.weekday() + 1, "meals": []})
            menu = menu.sudo()
